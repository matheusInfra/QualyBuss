...
export const dashboardService = {
    ...
    async fetchComplianceMetrics() {
        ...
        // 2. Collabs with at least one document
        // Note: Supabase doesn't support 'distinct' count easily in one go without RPC or raw SQL.
        // We'll fetch all document collaborator_ids (lightweight if we only sel ID) and Set() them.
        const { data: docs } = await supabase
            .from('collaborator_documents')
            .select('collaborator_id');

        const collabsWithDocs = new Set(docs?.map(d => d.collaborator_id)).size;

        // Compliance Rate (Simulated: Has at least 1 doc)
        const safeTotal = totalActive || 1;
        const complianceRate = Math.min(100, Math.round((collabsWithDocs / safeTotal) * 100));
        const missingDocsCount = Math.max(0, totalActive - collabsWithDocs);

        return {
            complianceRate,
            missingDocsCount,
            totalActive // Useful for charts
        };
    },
    ...
    async getKPIs() {
        try {
            const [collab, occur, absence, move, comp, audit] = await Promise.all([
